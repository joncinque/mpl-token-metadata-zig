...
pub const program_id = sol.PublicKey.comptimeFromBase58("metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s");
...
pub fn getMetadataId(mint_id: sol.PublicKey) !sol.PublicKey {
    const pda = try sol.PublicKey.findProgramAddress(.{ "metadata", &program_id.bytes, mint_id }, program_id);
    return pda.address;
}

pub fn getMasterEditionId(mint_id: sol.PublicKey) !sol.PublicKey {
    const pda = try sol.PublicKey.findProgramAddress(.{ "metadata", &program_id.bytes, mint_id, "edition" }, program_id);
    return pda.address;
}

pub fn getEditionId(mint_id: sol.PublicKey, edition_number: u8) !sol.PublicKey {
    const pda = try sol.PublicKey.findProgramAddress(.{ "metadata", &program_id.bytes, mint_id, "edition", &.{edition_number / edition_marker_bit_len} }, program_id);
    return pda.address;
}
